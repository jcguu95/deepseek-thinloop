...
### 8. 等 DeepSeek 時，估真實時長，別盲等圓整數
loop 是背景跑的，你會被它收尾（出 `status.json`）叫醒；中途要不要醒來檢查由你定。
**不要隨手挑「25 分鐘」這種圓整數** —— 那是偷懶不是估算。做法：
- **先估**：用便宜訊號量進度速率 —— `transcript.jsonl` 事件增量、workspace 改了哪些檔
  （`git status --porcelain`）、有沒有 `status.json`。看它走到哪一階段（探索 / 寫檔 /
  debug 綠燈 / verify），對照過往同類任務（如 limn-client ≈ 20 iter）推總長。
- **不確定就 bisect / 指數退避**：一開始不知道多久 → 由短而長地翻倍檢查（如 4→8→16 分），
  每次量 delta 收斂出剩餘時間，往「預期完成點之前一點」醒來，而不是賭一個大數字一次睡死。
- **省 token 的小細節**：<~270s 的檢查 prompt cache 還熱（早期快速 poll 便宜）；
  一旦把時長框住了，再拉長間隔。

盲等 = 要嘛太早醒（浪費）、要嘛太晚醒（拖到使用者）。估算與退避是紀律，不是按鈕。
